...
class Node(object):
    ...
    def make_sibling(self, sibling, parent):
        sib_parent = sibling.parent
        if sibling.parent:
            sib_gp = sibling.parent.parent
            old_sib = sibling.siblings()[0]

            old_sib.parent = sib_gp
            if sib_gp:
                sib_gp.children.remove(sib_parent)
                sib_gp.children.append(old_sib)

            # If we want to keep around deleted nodes, we would want to keep this edge:
            sib_parent.parent = None
            sib_parent.children = []
            sib_parent.deleted = True
            # other_parent.nsw_node.delete()
            sib_parent.pts = None
        else:
            assert len(sibling.children) == 0

        parent.parent = self.parent
        if parent.parent:
            parent.parent.children.remove(self)
