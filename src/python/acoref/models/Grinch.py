...
from acoref.models.GNode import GNode
...
class ACorefGrinch(object):
    ...
    def hallucinate_merge(self,n1, n2):
                ...
                if new_score > curr.parent.my_score:
                    if debug:
                        print('#FIND_INSERT-RETURN\tnew_score=%s\tcurr.parent.score=%s\tcurr_id=%s\tparent_id=%s\tcurr_num_pts=%s\tparent_num_pts=%s' % (
                            new_score, curr.parent.my_score, curr.id, curr.parent.id, curr.point_counter, curr.parent.point_counter))
                    break
                else:
                    if debug:
    ...
    def _try_graft(self, curr, offlimits, gnode, p_idx, graft_index, start_time):
        ...
        while curr != our_lca and other != our_lca and curr not in other.siblings():
            ...
            other_score = self.model.quick_e_score(curr.e_model,
                                                   other.e_model)
            i_like_you = other_score >= curr.parent.my_score
            you_like_me = other_score > other.parent.my_score

            print('#i_like_you and you_like me\t%s\t%s\t%s\t%s\t%s' % (i_like_you,
                                                           you_like_me,other_score,curr.parent.my_score,
                                                                       other.parent.my_score))
    ...
    def insert(self, p, p_idx):
        ...
        gnode = GNode([p], self.model.new(p[0]))
        ...
        if self.root is None:
            ...
            if self.perform_graft:
                ...
                graft_index = 0
                graft_index_limit = self.config.graft_beam
                perf_graft_last_time = False
                ...
                while curr and curr.parent and (perf_graft_last_time or graft_index < graft_index_limit):
                    if self.config.max_node_graft_size is not None:
                        if curr.point_counter > self.config.max_node_graft_size:
                            break
                    ...
                    if (self.config.fast_graft and graft_index == 0) or self.config.fast_grafts_only:
                    ...
                    if curr_new and curr_new.parent:
                        ...
                        # offlimits.update([x.nsw_node for x in (
                        #         [curr] + sib.descendants() + curr.siblings())]
                        # )
                        curr = curr_new
                        perf_graft_last_time = True
                        print("#performedGraft\t%s\t%s" % (p_idx,graft_index))
                    else:
                        perf_graft_last_time = False
                        if curr.parent:
                            offlimits.update(set([x.nsw_node for x in (
    ...
    def _try_graft_fast(self, curr, knn_and_score, offlimits, gnode, p_idx, graft_index, start_time):
        ...
        while curr != our_lca and other != our_lca and curr not in other.siblings():
            if self.config.max_node_graft_size is not None:
                if other.point_counter > self.config.max_node_graft_size:
                    break
            if self.config.debug:
                print('#tryGraftFast trying new parent\t%s\t%s' % (curr.id, other.id))
                sys.stdout.flush()
...
class ACorefPerch(ACorefGrinch):
    def __init__(self, config, dataset, model):
        super(ACorefPerch, self).__init__(config, dataset, model,
                                    perform_rotation=True,
                                    perform_graft=False)


class ACorefGreedy(ACorefGrinch):
    def __init__(self, config, dataset, model):
        super(ACorefGreedy, self).__init__(config, dataset, model,
