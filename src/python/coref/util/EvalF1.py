...
def eval_f1(config, predicted_cluster_assignment_file,gold_cluster_assignment_file,restrict_to_gold=False):
    ...
    outf = os.path.join(os.path.dirname(predicted_cluster_assignment_file), 'tmp-f1.tsv')
    os.system('set -exu; sh $XCLUSTER_ROOT/bin/util/score_pairwise.sh %s %s %s %s %s %s'
