...
class RexaBaseSubEnt(Module):
    """A pairwise author model (designed for Rexa)"""

    def __init__(self, config, vocab):
        super(RexaBaseSubEnt, self).__init__()
        self.config = config
        self.vocab = vocab

        self.pw_model = BasePairwiseModel(config,vocab)
        self.sub_ent_model = BaseSubEntModel(config, vocab)
    ...
    def pw_score(self, m1, m2):
        ...
        :param m1: a mention (util.Ment).
        :param m2: a mention (util.Ment).
        :return: torch.variable of the fit
        """
